...
if (!require("readr")) install.packages("mgcv")
...
model_list<-list()
for(i in 1:25){
  m = gam(Y_train[,1] ~ s(p1,p2,p3,p4,bs="gp",k=20) + s(q1,q2,q3,q4,bs="gp",k=20),data=X_train8) # k=2000 was used in practise
  model_list[[i]] = m
...
output_mse_results = LearnParam25.v(sqrt(starting_points),y=Y_test[1,],list_m=model_list,maxit=20,error_method="MSE",Cov_y=cov(cbind(Y_train))) 

# Low Rank GP, Output, Mahalanobis Distance, one starting pointn_starting_points = 10
output_mse_results = LearnParam25.v(sqrt(starting_points),y=Y_test[1,],list_m=model_list,maxit=20,error_method="Mahal",Cov_y=cov(cbind(Y_train))) 
...
# Results for loss method need squaring
# also need to identify which is the lowest optimised loss
